...
type OVSNetworkSpec struct {
	...
	// +kubebuilder:validation:Maximum=65279
	// OpenFlow port number in range 1 to 65,279
	OfportRequest uint `json:"ofportRequest,omitempty"`
	// The type of interface on ovs.
	InterfaceType string `json:"interfaceType,omitempty"`
	// Path to the configuration file on the host
	ConfigurationPath string `json:"configurationPath,omitempty"`
	// Paths to OVSDB socket
	SocketFile string `json:"socketFile,omitempty"`
	// Amount of retries for link state check
	LinkStateCheckRetries uint `json:"linkStateCheckRetries,omitempty"`
	// Interval between retries for link state check
	LinkStateCheckInterval uint `json:"linkStateCheckInterval,omitempty"`
}
