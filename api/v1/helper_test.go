...
func TestNeedToUpdateInterfaceEswitchMode(t *testing.T) {
	testtable := []struct {
		tname          string
		spec           *v1.Interface
		...
		},
		{
			tname:          "not match",
			spec:           &v1.Interface{EswitchMode: v1.ESwithModeSwitchDev},
			status:         &v1.InterfaceExt{EswitchMode: v1.ESwithModeLegacy},
			expectedResult: true,
		},
		{
			tname:          "match",
			spec:           &v1.Interface{EswitchMode: v1.ESwithModeSwitchDev},
			status:         &v1.InterfaceExt{EswitchMode: v1.ESwithModeSwitchDev},
			expectedResult: false,
		},
	}
	for _, tc := range testtable {
		t.Run(tc.tname, func(t *testing.T) {
			result := v1.NeedToUpdateInterfaceEswitchMode(tc.spec, tc.status)
			if result != tc.expectedResult {
				t.Errorf("unexpected result: expected %t, actual %t", tc.expectedResult, result)
			}
		})
	}
}
